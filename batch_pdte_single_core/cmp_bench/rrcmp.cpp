...
//g++ -o rrcmp -O3 rrcmp.cpp ../src/cmp.cpp -I ../include -I /usr/local/include/SEAL-4.1 -lseal-4.1
...
int main(int argc, char* argv[]){
    ...
    EncryptionParameters parms = rrcmp_init(n);
    ...
    if(n < 64){
        ...
        encrypted_op_encode = rrcmp_encode_b(encrypted_op,n,slot_count,row_count,num_cmps_per_row);
        Plaintext plaintext; 
        batch_encoder->encode(encrypted_op_encode, plaintext);
        encryptor->encrypt(plaintext, client_input);

        plain_op_encode = rrcmp_encode_a(plain_op,num_slots_per_element,slot_count,row_count,num_cmps_per_row);
    ...
    clock_t start=clock();
    Ciphertext result = rrcmp(evaluator,gal_keys_server,rlk_server,num_slots_per_element,server_input,client_input);
    ...
    vector<uint64_t> ans = rrcmp_decode_a_gt_b_dec(result,decryptor,batch_encoder,num_cmps,num_slots_per_element,num_cmps_per_row,row_count);
