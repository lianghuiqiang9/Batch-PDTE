...
//g++ -o lrcmp -O3 lrcmp.cpp ../src/cmp.cpp -I ../include -I /usr/local/include/SEAL-4.1 -lseal-4.1
...
int main(int argc, char* argv[]){
    ...
    EncryptionParameters parms = lrcmp_init(n);
    //BCMP初始化公私钥
    ...
    if(n <= 64){
        ...
        encrypted_op_encode = lrcmp_encode_b(encrypted_op,n,slot_count,row_count);
        Plaintext plaintext; 
        for(int i = 0 ; i < n; i++){
        ...
        //
        plain_op_encode = lrcmp_encode_a(plain_op,n,slot_count,row_count);
        for(int i = 0 ; i < n; i++){
    ...
    clock_t start = clock();
    Ciphertext comparisonResult = lrcmp(evaluator,rlk_server,n,server_input,client_input);
    auto finish = clock()-start;

    server_send_commun+=comparisonResult.save(server_send);

    Plaintext pt;
    decryptor->decrypt(comparisonResult, pt);
    vector<uint64_t> ans;
    batch_encoder->decode(pt, ans);
